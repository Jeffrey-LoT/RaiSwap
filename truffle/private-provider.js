...
function PrivateKeyProvider(privateKey, providerUrl, chainId) {
    ...
    this.wallet = EthereumjsWallet.default.fromPrivateKey(
       new Buffer.from(privateKey[0], 'hex')
    );
    ...
    for (let i = 0; i < privateKey.length; i++) {
        wallets.push(
            EthereumjsWallet.default.fromPrivateKey(
                new Buffer.from(privateKey[i], "hex")
            )
        );
        addresses.push("0x" + wallets[i].getAddress().toString("hex"));
    }
